...
- **Open problems N1–N7** — wall-clock kill-switch, goal auto-promotion, token accounting race, self-healing oracle-patching, unsatisfiable shell oracles, dead judge config, invisible stdout/stderr logs.
  **Fixed and verified 2026-07-06**: Added Judge entailment check, frontier check, and minimum structure requirement to avoid false completion (N2); made duration safety cap and relaunch limits configurable, defaulting to provider-aware values (N1); integrated monitor's budget alert in the supervisor to terminate on token budget exhaustion (N1); accumulated output tokens in memory instead of direct file read-modify-write to prevent lost updates (N7); disabled self-healing and spec-persist for numeric/symbolic oracles (N3); steered proposer prompt away from shell oracles on proof runs (N5); plumed `--judge-provider` and `--judge-model` to all components, adding largest local model auto-detection (N4); enabled unbuffered subprocess execution and logging to `.proximity_swarm/logs/` (N6). All 107 unit tests pass.
